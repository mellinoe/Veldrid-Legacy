...
namespace Veldrid.Graphics.Direct3D
{
    ...
    //    {
    //        SetArrayDataCore(data, 0, Unsafe.SizeOf<T>() * data.Length, 0);
    //    }

    //    public void SetData<T>(ArraySegment<T> data, int destinationOffsetInBytes) where T : struct
    //    {
    //        int dataSizeInBytes = Unsafe.SizeOf<T>() * data.Count;
    //        SetArrayDataCore(data.Array, data.Offset, dataSizeInBytes, destinationOffsetInBytes);
    //    }

    //    public void GetData<T>(T[] storageLocation) where T : struct
    //    {
    //        GetData(storageLocation, Unsafe.SizeOf<T>() * storageLocation.Length);
    //    }
    //}
    ...
    public abstract class D3DBuffer : DeviceBufferBase
    {
        ...
        public unsafe override void SetData(IntPtr data, int dataSizeInBytes, int destinationOffsetInBytes)
        {
            ...
            else
            {
                ...
                DataBox dataBox = new DataBox(data);
                Device.ImmediateContext.UpdateSubresource(dataBox, Buffer, 0, region: subregion);
            }
        }
